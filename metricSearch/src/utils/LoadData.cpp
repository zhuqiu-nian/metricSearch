...
DataList loadUMADData(int data_var, int data_num) {
    ...
    case 4: { // ← 新增：图片特征
        auto imageData = loadImageFeatureData(selectedFile, data_num);
        result.assign(imageData.begin(), imageData.end());
        std::cout << "[加载完成] 已加载图片特征数据集 '" << files[choice - 1] << "', 共 " << imageData.size() << " 条。\n";
        break;
    }
    }
...
// ========== 新增：加载图片特征数据 ==========
vector<shared_ptr<ImageFeatureData>> loadImageFeatureData(const string& filename, int num_images = -1) {
    vector<shared_ptr<ImageFeatureData>> dataList;
    ...
    cout << "[信息] 正在加载图片特征数据集: " << filename << endl;
    ...
    while (getline(file, line)) {
        ...
        // 如果指定了数量且已满足，则跳出
        if (num_images >= 0 && successCount >= num_images) {
            break;
        }
        ...
        string filenamePart;
        if (!(iss >> filenamePart)) {
            cerr << "[警告] 第 " << lineNumber << " 行：无法读取文件名。\n";
            continue;
        }
        ...
        // 检查是否恰好 66 维
        if (features.size() != 66) {
            cerr << "[警告] 第 " << lineNumber << " 行：特征维度为 " << features.size()
                << "，跳过（应为 66）。\n";
            continue;
        }

        try {
            auto data = make_shared<ImageFeatureData>(filenamePart, features);
            dataList.push_back(data);
            successCount++;
        }
        catch (const exception& e) {
            cerr << "[警告] 第 " << lineNumber << " 行：创建 ImageFeatureData 失败: "
                << e.what() << "\n";
        }
    }

    file.close();
    cout << "[完成] 成功加载 " << successCount << " 张图片的特征数据。\n";
    return dataList;
}
