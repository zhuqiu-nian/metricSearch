...
public:
    ...
    static shared_ptr<MetricDistance> createDistanceFunction(int distanceType, int data_var) {
        if (data_var == 1) {
            ...
            // 图片特征数据：结构(3)+颜色(15)+纹理(48)
            // 距离 = (L2_struct + L1_color + L2_texture) / 3
            // 只支持一种距离函数，distanceType 应为 1（可忽略）
            if (distanceType != 1) {
                cerr << "[提示] 图片特征数据仅支持 distanceType = 1（使用 ImageFeatureDistance）\n";
            }
            return make_shared<ImageFeatureDistance>();
        }
        else {
